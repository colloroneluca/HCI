...
class hand_gesture_browser():
    ...
    def get_screenshot(self, img, b):
        ...
        if length < 50 and time.time() - self.last_click_time > 1.5:
            if self.screenshot_guard==2:
                self.last_click_time = time.time()
                b.get_browser_screenshot()
                start_sound("screenshot.mp3", 1)
                self.screenshot_guard = 0
            else:
                self.screenshot_guard+=1
        elif self.screenshot_guard>0:
            self.screenshot_guard -=1
    ...
    def close(self, b):
        ...
        PROCNAME = "python3"
        PROCNAME2 = "main.pyw"
        f = open("/home/rodja/HCI/log.txt", "a")
